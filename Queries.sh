...
echo "$($PSQL "SELECT name FROM teams t FULL JOIN games g ON t.team_id = g.winner_id or t.team_id = g.opponent_id WHERE round='Eighth-Final' AND year = 2014 ORDER BY t.name;")"
...
echo "$($PSQL "SELECT year,t.name FROM games g INNER JOIN teams t ON g.winner_id = t.team_id WHERE round = 'Final' ORDER BY year;")"
